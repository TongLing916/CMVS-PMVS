...
class CphotoSetS {
 public:
  ...
  void grabTex(const int index, const int level, const Vec2f& icoord,
               const Vec2f& xaxis, const Vec2f& yaxis, std::vector<Vec3f>& tex,
  ...
  inline Vec3f project(const int index, const Vec4f& coord,
  ...
  inline int getMask(const Vec4f& coord, const int index,
                     const int level) const;
  ...
  inline int getEdge(const Vec4f& coord, const int index,
                     const int level) const;
...
int CphotoSetS::getMask(const Vec4f& coord, const int level) const {
  for (int index = 0; index < m_num; ++index)
    if (getMask(coord, index, level) == 0) return 0;
  return 1;
};
