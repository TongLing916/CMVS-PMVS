...
void CphotoSetS::init(const std::vector<int>& images, const std::string prefix,
  ...
  m_images = images;
  m_num = static_cast<int>(images.size());
...
void CphotoSetS::getPAxes(const int index, const Vec4f& coord,
                          const Vec4f& normal, Vec4f& pxaxis,
                          Vec4f& pyaxis) const {
...
void CphotoSetS::grabTex(const int index, const int level, const Vec4f& coord,
  ...
  m_photos[index].grabTex(level, coord, pxaxis, pyaxis, pzaxis, m_size, tex,
                          weight, normalizef);
}
...
void CphotoSetS::getMinMaxAngles(const Vec4f& coord,
                                 const std::vector<int>& indexes,
                                 float& minAngle, float& maxAngle) const {
  minAngle = M_PI;
  maxAngle = 0.0f;
  std::vector<Vec4f> rays;
  rays.resize(static_cast<int>(indexes.size()));
...
int CphotoSetS::checkAngles(const Vec4f& coord, const std::vector<int>& indexes,
