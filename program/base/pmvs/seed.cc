...
void Cseed::run() {
  ...
  cerr << "done" << endl;
  time(&tv);
  cerr << "---- Initial: " << (tv - curtime) / CLOCKS_PER_SEC << " secs ----"
       << endl;
...
void Cseed::clear() { vector<vector<vector<Ppoint> > >().swap(m_ppoints); }
    ...
    for (int x = 0; x < gwidth; ++x) {
      ...
      for (int p = 0; p < (int)m_ppoints[index][index2].size(); ++p) {
        ...
        for (int i = 0; i < (int)vcp.size(); ++i) {
          ...
          const int ix =
              ((int)floor(vcp[i]->m_icoord[0] + 0.5f)) / m_fm.m_csize;
          const int iy =
...
void Cseed::collectCells(const int index0, const int index1, const Cpoint& p0,
  ...
  Mat3 F;
  Image::setF(m_fm.m_pss.m_photos[index0], m_fm.m_pss.m_photos[index1], F,
              m_fm.m_level);
  ...
  if (line[0] == 0.0 && line[1] == 0.0) {
    cerr << "Point right on top of the epipole?" << index0 << ' ' << index1
         << endl;
    return;
  }
  // vertical
...
void Cseed::collectCandidates(const int index, const std::vector<int>& indexes,
  ...
  for (int i = 0; i < (int)indexes.size(); ++i) {
    ...
    Mat3 F;
    Image::setF(m_fm.m_pss.m_photos[index], m_fm.m_pss.m_photos[indexid], F,
                m_fm.m_level);
...
void Cseed::unproject(const int index0, const int index1, const Cpoint& p0,
                      const Cpoint& p1, Vec4f& coord) const {
  Mat4 A;
  A[0][0] = m_fm.m_pss.m_photos[index0].m_projection[m_fm.m_level][0][0] -
            p0.m_icoord[0] *
                m_fm.m_pss.m_photos[index0].m_projection[m_fm.m_level][2][0];
  A[0][1] = m_fm.m_pss.m_photos[index0].m_projection[m_fm.m_level][0][1] -
            p0.m_icoord[0] *
                m_fm.m_pss.m_photos[index0].m_projection[m_fm.m_level][2][1];
  A[0][2] = m_fm.m_pss.m_photos[index0].m_projection[m_fm.m_level][0][2] -
            p0.m_icoord[0] *
                m_fm.m_pss.m_photos[index0].m_projection[m_fm.m_level][2][2];
  ...
  A[2][0] = m_fm.m_pss.m_photos[index1].m_projection[m_fm.m_level][0][0] -
            p1.m_icoord[0] *
                m_fm.m_pss.m_photos[index1].m_projection[m_fm.m_level][2][0];
  A[2][1] = m_fm.m_pss.m_photos[index1].m_projection[m_fm.m_level][0][1] -
            p1.m_icoord[0] *
                m_fm.m_pss.m_photos[index1].m_projection[m_fm.m_level][2][1];
  A[2][2] = m_fm.m_pss.m_photos[index1].m_projection[m_fm.m_level][0][2] -
            p1.m_icoord[0] *
                m_fm.m_pss.m_photos[index1].m_projection[m_fm.m_level][2][2];
  ...
  Vec4 b;
  b[0] = p0.m_icoord[0] *
             m_fm.m_pss.m_photos[index0].m_projection[m_fm.m_level][2][3] -
         m_fm.m_pss.m_photos[index0].m_projection[m_fm.m_level][0][3];
  b[1] = p0.m_icoord[1] *
             m_fm.m_pss.m_photos[index0].m_projection[m_fm.m_level][2][3] -
         m_fm.m_pss.m_photos[index0].m_projection[m_fm.m_level][1][3];
  b[2] = p1.m_icoord[0] *
             m_fm.m_pss.m_photos[index1].m_projection[m_fm.m_level][2][3] -
