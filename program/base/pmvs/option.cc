...
void Soption::init(const std::string& prefix, const std::string& option) {
    ...
    if (name == "level") {
      ...
      if (m_tflag == -1) {
        ...
        for (int i = firstimage; i < lastimage; ++i) {
          m_timages.emplace_back(i);
        }
      } else if (0 < m_tflag) {
        for (int i = 0; i < m_tflag; ++i) {
          int itmp;
          ifstr >> itmp;
          m_timages.emplace_back(itmp);
        }
      } else {
        std::cerr << "tflag is not valid: " << m_tflag << std::endl;
        exit(1);
      }
      ...
      if (m_oflag == -1) {
        ...
        ifstr >> firstimage >> lastimage;
        for (int i = firstimage; i < lastimage; ++i) {
          m_oimages.emplace_back(i);
        }
      } else if (0 <= m_oflag) {
        for (int i = 0; i < m_oflag; ++i) {
          int itmp;
          ifstr >> itmp;
          m_oimages.emplace_back(itmp);
        }
      } else if (m_oflag != -2 && m_oflag != -3) {
        std::cerr << "oflag is not valid: " << m_oflag << std::endl;
        exit(1);
      }
    } else if (name == "quad") {
      ...
      std::cerr << "Unrecognizable option: " << name << std::endl;
      exit(1);
    }
  }
  ifstr.close();
  ...
  if (m_tflag == -10 || m_oflag == -10) {
    std::cerr << "m_tflag and m_oflag not specified: " << m_tflag << ' '
              << m_oflag << std::endl;
    exit(1);
  }
  ...
  for (int i = 0; i < static_cast<int>(m_timages.size()); ++i) {
  ...
  std::cerr << "# of timages: " << m_timages.size();
  if (m_tflag == -1) {
    std::cerr << " (range specification)" << std::endl;
  } else {
    std::cerr << " (enumeration)" << std::endl;
  }
  std::cerr << "# of oimages: " << m_oimages.size();
  if (m_oflag == -1) {
    std::cerr << " (range specification)" << std::endl;
  } else if (0 <= m_oflag) {
    std::cerr << " (enumeration)" << std::endl;
  ...
  std::cerr << "level: " << m_level << "  csize: " << m_csize << std::endl
            << "threshold: " << m_threshold << "  wsize: " << m_wsize
            << std::endl
            << "minImageNum: " << m_minImageNum << "  CPU: " << m_CPU
            << std::endl
            << "useVisData: " << m_useVisData << "  sequence: " << m_sequence
            << std::endl;
  std::cerr << "--------------------------------------------------"
            << std::endl;
}
  ...
  for (int c = 0; c < num2; ++c) {
    ...
    for (int i = 0; i < itmp; ++i) {
      ...
      if (index0 != -1 && m_dict.find(itmp2) == m_dict.end())
        m_oimages.emplace_back(itmp2);
    }
  }
  ifstr.close();

  std::sort(m_oimages.begin(), m_oimages.end());
  m_oimages.erase(unique(m_oimages.begin(), m_oimages.end()), m_oimages.end());
}

// When do not use vis.dat
void Soption::initVisdata() {
  // Case classifications. Set m_visdata by using vis.dat or not.
  if (m_useVisData == 0) {
    const int tnum = static_cast<int>(m_timages.size());
    const int onum = static_cast<int>(m_oimages.size());
  ...
  for (int c = 0; c < num2; ++c) {
    ...
    for (int i = 0; i < itmp; ++i) {
      ...
      if (index0 != -1 && index1 != -1) {
        m_visdata2[index0].emplace_back(index1);
      }
    }
  }
  ifstr.close();

  const int num = static_cast<int>(images.size());
  m_visdata.clear();
  m_visdata.resize(num);
  for (int y = 0; y < num; ++y) {
    m_visdata[y].resize(num);
    fill(m_visdata[y].begin(), m_visdata[y].end(), 0);
    for (int x = 0; x < static_cast<int>(m_visdata2[y].size()); ++x)
      m_visdata[y][m_visdata2[y][x]] = 1;
  }

  // check symmetry
  for (int i = 0; i < static_cast<int>(m_visdata.size()); ++i) {
    for (int j = i + 1; j < static_cast<int>(m_visdata.size()); ++j) {
...
void Soption::initBindexes(const std::string sbimages) {
  ...
  for (int i = 0; i < itmp; ++i) {
    ...
    if (m_dict.find(itmp0) != m_dict.end()) {
      m_bindexes.emplace_back(m_dict[itmp0]);
    }
  }
  ifstr.close();
}
