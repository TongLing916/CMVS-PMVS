...
class CfindMatch {
 public:
  ...
  void run();
  void write(const std::string prefix, bool bExportPLY, bool bExportPatch,
             bool bExportPSet);
  ...
  /** \brief If patches are dense or not
   *
   *  that is, if we use check(patch) after patch optimization
   *
  */
  int m_depth;
  ...
  // Thresholds
  //----------------------------------------------------------------------
  /** \brief For first feature matching.
   *
   *  Images within this angle are used in matching.
  */
