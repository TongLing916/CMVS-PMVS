...
void Coptim::setAxesScales() {
  ...
  for (int index = 0; index < m_fm.m_num; ++index) {
    const Vec4f xaxe(m_xaxes[index][0], m_xaxes[index][1], m_xaxes[index][2],
                     0.0);
    const Vec4f yaxe(m_yaxes[index][0], m_yaxes[index][1], m_yaxes[index][2],
                     0.0);
...
int Coptim::preProcess(Cpatch& patch, const int id, const int seed) {
  ...
  const int flag = m_fm.m_pss.checkAngles(
      patch.m_coord, patch.m_images, m_fm.m_maxAngleThreshold,
      m_fm.m_angleThreshold1, m_fm.m_minImageNumThreshold);
...
int Coptim::postProcess(Cpatch& patch, const int id, const int seed) {
  ...
  while (begin != end) {
    if (*begin < m_fm.m_tnum) ++patch.m_timages;
    ++begin;
  }
...
void Coptim::addImages(Patch::Cpatch& patch) const {
  ...
  for (int index = 0; index < m_fm.m_num; ++index) used[index] = 0;
...
void Coptim::computeUnits(const Patch::Cpatch& patch, std::vector<int>& indexes,
...
double Coptim::my_f(unsigned n, const double* x, double* grad,
                    void* my_func_data) {
  ...
  for (int i = 0; i < size; ++i) {
    int flag;
    flag =
        m_one->grabTex(coord, pxaxis, pyaxis, normal, m_one->m_indexesT[id][i],
  ...
  if (pairwise) {
    ...
    for (int i = 0; i < size; ++i) {
      for (int j = i + 1; j < size; ++j) {
        ...
        ans += robustincc(
            1.0 - m_one->dot(m_one->m_texsT[id][i], m_one->m_texsT[id][j]));
        denom++;
      }
    }
    if (denom <
    ...
    if (m_one->m_texsT[id][0].empty()) return 2.0;
    ...
    for (int i = 1; i < size; ++i) {
      ...
      ans += robustincc(
          1.0 - m_one->dot(m_one->m_texsT[id][0], m_one->m_texsT[id][i]));
      denom++;
    }
...
bool Coptim::refinePatchBFGS(Cpatch& patch, const int id, const int time,
                             const int ncc) {
  int idtmp = id;
  ...
  if (success) {
    ...
    patch.m_ncc = 1.0 - unrobustincc(computeINCC(patch.m_coord, patch.m_normal,
                                                 patch.m_images, id, 1));
...
void Coptim::encode(const Vec4f& coord, double* const vect,
                    const int id) const {
  vect[0] = (coord - m_centersT[id]) * m_raysT[id] / m_dscalesT[id];
}

void Coptim::encode(const Vec4f& coord, const Vec4f& normal, double* const vect,
                    const int id) const {
  ...
  const float fx =
      m_xaxes[image] *
      proj(normal);  // projects from 4D to 3D, divide by last value
...
void Coptim::decode(Vec4f& coord, Vec4f& normal, const double* const vect,
                    const int id) const {
...
void Coptim::decode(Vec4f& coord, const double* const vect,
                    const int id) const {
  coord = m_centersT[id] + m_dscalesT[id] * vect[0] * m_raysT[id];
}

void Coptim::setINCCs(const Patch::Cpatch& patch, std::vector<float>& inccs,
...
void Coptim::setINCCs(const Patch::Cpatch& patch,
  ...
  inccs.resize(size);
  for (int i = 0; i < size; ++i) inccs[i].resize(size);
...
// My own optimisaton
float MyPow2(int x) {
  const float answers[] = {0.0625, 0.125, 0.25, 0.5, 1,   2,   4,   8,
...
int Coptim::grabTex(const Vec4f& coord, const Vec4f& pxaxis,
                    const Vec4f& pyaxis, const Vec4f& pzaxis, const int index,
                    const int size, std::vector<float>& tex) const {
  tex.clear();
...
double Coptim::computeINCC(const Vec4f& coord, const Vec4f& normal,
  ...
  for (int i = 0; i < size; ++i) {
    int flag;
    flag = grabTex(coord, pxaxis, pyaxis, normal, indexes[i], m_fm.m_wsize,
                   texs[i]);
...
void Coptim::func(int m, int n, double* x, double* fvec, int* iflag,
                  void* arg) {
...
void Coptim::normalize(std::vector<std::vector<float> >& texs, const int size) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    Vec3f scale;
    for (int j = 0; j < 3; ++j)
      if (rgbs[i][j] != 0.0f) scale[j] = ave[j] / rgbs[i][j];
...
void Coptim::getPAxes(const int index, const Vec4f& coord, const Vec4f& normal,
  ...
  pyaxis *= pscale;
  const float xdis =
      norm(m_fm.m_pss.project(index, coord + pxaxis, m_fm.m_level) -
           m_fm.m_pss.project(index, coord, m_fm.m_level));
  const float ydis =
      norm(m_fm.m_pss.project(index, coord + pyaxis, m_fm.m_level) -
           m_fm.m_pss.project(index, coord, m_fm.m_level));
  pxaxis /= xdis;
  pyaxis /= ydis;
}
