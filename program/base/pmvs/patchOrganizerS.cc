...
void CpatchOrganizerS::image2index(Cpatch& patch) {
  ...
  for (int i = 0; i < (int)patch.m_images.size(); ++i) {
    ...
    if (index != -1) newimages.emplace_back(index);
  }
...
void CpatchOrganizerS::init() {
  ...
  for (int index = 0; index < m_fm.m_num; ++index) {
    const int gwidth =
        (m_fm.m_pss.getWidth(index, m_fm.m_level) + m_fm.m_csize - 1) /
        m_fm.m_csize;
    const int gheight =
        (m_fm.m_pss.getHeight(index, m_fm.m_level) + m_fm.m_csize - 1) /
        m_fm.m_csize;
...
void CpatchOrganizerS::writePatches2(const std::string prefix, bool bExportPLY,
  ...
  if (bExportPSet) {
    ...
    ofstr.open(buffer);
    for (int p = 0; p < (int)m_ppatches.size(); ++p)
      ofstr << m_ppatches[p]->m_coord[0] << ' ' << m_ppatches[p]->m_coord[1]
            << ' ' << m_ppatches[p]->m_coord[2] << ' '
            << m_ppatches[p]->m_normal[0] << ' ' << m_ppatches[p]->m_normal[1]
...
void CpatchOrganizerS::readPatches() {
  ...
  for (int i = 0; i < m_fm.m_tnum; ++i) {
    ...
    char buffer[1024];
    sprintf(buffer, "%smodels/%08d.patc%d", m_fm.m_prefix.c_str(), image,
            m_fm.m_level);
    ifstream ifstr;
    ifstr.open(buffer);
...
void CpatchOrganizerS::collectPatches(
...
void CpatchOrganizerS::collectNonFixPatches(
    const int index, std::vector<Patch::Ppatch>& ppatches) {
  const std::lock_guard<std::mutex> lock(m_fm.m_imageLocks[index]);
  for (int i = 0; i < (int)m_pgrids[index].size(); ++i) {
    vector<Ppatch>::iterator begin = m_pgrids[index][i].begin();
    vector<Ppatch>::iterator end = m_pgrids[index][i].end();
...
void CpatchOrganizerS::addPatch(Patch::Ppatch& ppatch) {
  ...
  while (bimage != eimage) {
    ...
    {
      const std::lock_guard<std::mutex> lock(m_fm.m_imageLocks[index]);
      m_pgrids[index][index2].emplace_back(ppatch);
    }
    ++bimage;
    ++bgrid;
  }
  ...
  while (bimage != eimage) {
    ...
    {
      const std::lock_guard<std::mutex> lock(m_fm.m_imageLocks[index]);
      m_vpgrids[index][index2].emplace_back(ppatch);
    }
    ++bimage;
    ++bgrid;
  }
...
void CpatchOrganizerS::updateDepthMaps(Ppatch& ppatch) {
  for (int image = 0; image < m_fm.m_tnum; ++image) {
    const Vec3f icoord =
        m_fm.m_pss.project(image, ppatch->m_coord, m_fm.m_level);
...

    {
      ...
      for (int j = 0; j < 2; ++j) {
        for (int i = 0; i < 2; ++i) {
          if (xs[i] < 0 || m_gwidths[image] <= xs[i] || ys[j] < 0 ||
              m_gheights[image] <= ys[j])
            continue;
...
void CpatchOrganizerS::setGridsImages(Patch::Cpatch& patch,
  ...
  while (bimage != eimage) {
    ...
    if (0 <= ix && ix < m_gwidths[*bimage] && 0 <= iy &&
      ...
      patch.m_grids.emplace_back(Vec2i(ix, iy));
    }
    ++bimage;
  }
}
...
void CpatchOrganizerS::setGrids(Cpatch& patch) const {
  ...
  for (int i = 0; i < (int)patch.m_images.size(); ++i) {
    ...
    const int ix = ((int)floor(icoord[0] + 0.5f)) / m_fm.m_csize;
    const int iy = ((int)floor(icoord[1] + 0.5f)) / m_fm.m_csize;
    patch.m_grids.emplace_back(TVec2<int>(ix, iy));
  }
}
...
void CpatchOrganizerS::setVImagesVGrids(Cpatch& patch) {
  ...
  for (int image = 0; image < m_fm.m_tnum; ++image) {
    ...
    patch.m_vimages.emplace_back(image);
    patch.m_vgrids.emplace_back(TVec2<int>(ix, iy));
  }
}
...
int CpatchOrganizerS::isVisible0(const Cpatch& patch, const int image, int& ix,
                                 int& iy, const float strict, const int lock) {
...
int CpatchOrganizerS::isVisible(const Cpatch& patch, const int image,
...

  {
    ...
    if (lock) guard.emplace_back(m_fm.m_imageLocks[image]);
  ...
  if (diff < m_fm.m_optim.getUnit(image, patch.m_coord) * m_fm.m_csize *
                 strict * factor)
    return 1;
  else
    return 0;
...
void CpatchOrganizerS::findNeighbors(const Patch::Cpatch& patch,
                                     ...
                                     const int margin, const int skipvis) {
        ...
        for (int i = -margin; i <= margin; ++i) {
          ...
          while (bpatch != epatch) {
            if (m_fm.isNeighborRadius(patch, **bpatch, unit,
                                      m_fm.m_neighborThreshold * scale, radius))
              neighbors.emplace_back(*bpatch);
            ++bpatch;
          }
...
void CpatchOrganizerS::setScales(Patch::Cpatch& patch) const {
  ...
  for (int i = 1; i < inum; ++i) {
    Vec3f diff =
        ...
        m_fm.m_pss.project(patch.m_images[i], patch.m_coord - ray * unit2,
                           m_fm.m_level);
...
void CpatchOrganizerS::writePLY(const std::vector<Ppatch>& patches,
  ...
  while (bpatch != bend) {
    ...
    ofstr << (*bpatch)->m_coord[0] << ' ' << (*bpatch)->m_coord[1] << ' '
          << (*bpatch)->m_coord[2] << ' ' << (*bpatch)->m_normal[0] << ' '
          << (*bpatch)->m_normal[1] << ' ' << (*bpatch)->m_normal[2] << ' '
