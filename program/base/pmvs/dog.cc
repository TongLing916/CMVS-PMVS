...
float Cdog::getResponse(const std::vector<std::vector<float> >& dog,
  ...
  const float sum = dog[y - 1][x - 1] + dog[y][x - 1] + dog[y + 1][x - 1] +
                    dog[y - 1][x] + dog[y + 1][x] + dog[y - 1][x + 1] +
...
void Cdog::run(const std::vector<unsigned char>& image,
  ...
  */
  ...
  const int steps =
      max(4, (int)ceil(log(m_lastScale / m_firstScale) / log(scalestep)));
  ...
  for (int i = 2; i <= steps - 1; ++i) {
    ...
    for (int y = margin; y < m_height - margin; ++y) {
      for (int x = margin; x < m_width - margin; ++x) {
        if (alreadydetected[y][x] || cdog[y][x] == 0.0) {
          continue;
        }
  ...
  cerr << result.size() << " dog done" << endl;
}
